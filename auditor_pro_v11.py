...
class AuditApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Auditor Pro v11.1 - BOM Validator (Bogdan Bahrim)")
        ...
        # --- EXPORT SETTINGS (CU COADĂ/SUFFIX) ---
        export_frame = tk.LabelFrame(root, text=" 💾 Export Settings (Base Name + Suffix) ", bg="#f4f7f6", padx=20, pady=10)
        export_frame.pack(fill=tk.X, padx=20, pady=10)

        # Rând pentru PDF
        tk.Label(export_frame, text="PDF Base Name:", bg="#f4f7f6").grid(row=0, column=0, sticky=tk.W)
        self.pdf_name_var = tk.StringVar(value="Audit_Drawing_Result")
        tk.Entry(export_frame, textvariable=self.pdf_name_var, width=30).grid(row=0, column=1, padx=5)
        
        tk.Label(export_frame, text="+ Suffix:", bg="#f4f7f6").grid(row=0, column=2, padx=5)
        self.pdf_suffix_var = tk.StringVar(value="_Rev0")
        tk.Entry(export_frame, textvariable=self.pdf_suffix_var, width=15).grid(row=0, column=3, padx=5)

        # Rând pentru Excel
        tk.Label(export_frame, text="Excel Base Name:", bg="#f4f7f6").grid(row=1, column=0, sticky=tk.W, pady=5)
        self.xlsx_name_var = tk.StringVar(value="Audit_Final_Report")
        tk.Entry(export_frame, textvariable=self.xlsx_name_var, width=30).grid(row=1, column=1, padx=5)
        
        tk.Label(export_frame, text="+ Suffix:", bg="#f4f7f6").grid(row=1, column=2, padx=5)
        self.xlsx_suffix_var = tk.StringVar(value="_Final")
        tk.Entry(export_frame, textvariable=self.xlsx_suffix_var, width=15).grid(row=1, column=3, padx=5)

        self.progress = ttk.Progressbar(root, orient=tk.HORIZONTAL, length=1200, mode='determinate')
        self.progress.pack(pady=10)
        self.status_var = tk.StringVar(value="Suntem în 2026. Gata de treabă.")
    ...
    def load_excel(self):
        ...
        lb = tk.Listbox(popup, selectmode="multiple", width=60, height=10)
        ...
        def confirm():
            ...
            for i in lb.curselection():
                ...
                id_col, pn_col = cols[0], next((c for c in cols if "PART" in str(c).upper() or "P/N" in str(c).upper()), None)
                desc_col = next((c for c in cols if "DESC" in str(c).upper()), None)
                qty_col = next((c for c in cols if "QTY" in str(c).upper()), None)
                for _, row in df.iterrows():
                    tag_val, pn_val = str(row[id_col]).strip(), str(row[pn_col]).strip() if pn_col else "-"
                    if tag_val and tag_val != "nan" and "TOTAL" not in tag_val.upper():
                        ...
                        self.full_results.append({
                            "sheet": sheet_name, "identifier": tag_val, "term_to_search": search_term,
                            "part_no": pn_val, "desc": str(row[desc_col]) if desc_col else "-",
                            "target": int(row[qty_col]) if qty_col and pd.notnull(row[qty_col]) else 1,
                            "hits": 0, "pages": [], "verdict": "Pending"
                        })
            self.refresh_table(); self.run_btn.config(state=tk.NORMAL) if self.pdf_path else None; popup.destroy()
        tk.Button(popup, text="Confirm", command=confirm).pack(pady=10)

    def load_pdf(self):
        path = filedialog.askopenfilename(filetypes=[("PDF", "*.pdf")])
        if path: self.pdf_path = path; self.run_btn.config(state=tk.NORMAL) if self.full_results else None
    ...
    def process(self):
        try:
            ...
            excluded = set()
            try:
                for p in self.exclude_entry.get().replace(" ", "").split(","):
                    if "-" in p:
                        s, e = map(int, p.split("-"))
            ...
            for i, item in enumerate(self.full_results):
                count, pgs = 0, []
                if item["term_to_search"] != "-":
                    for p_idx in range(len(doc)):
                        if p_idx in excluded: continue
                        matches = doc[p_idx].search_for(item["term_to_search"])
                        if matches:
                            count += len(matches); pgs.append(p_idx + 1)
                            for rect in matches:
            ...
            # LOGICA DE NUME + COADĂ
            folder = os.path.dirname(self.pdf_path)
            full_pdf_name = f"{self.pdf_name_var.get()}{self.pdf_suffix_var.get()}.pdf"
            full_xlsx_name = f"{self.xlsx_name_var.get()}{self.xlsx_suffix_var.get()}.xlsx"
            
            doc.save(os.path.join(folder, full_pdf_name))
            pd.DataFrame(self.full_results).drop(columns=['term_to_search']).to_excel(os.path.join(folder, full_xlsx_name), index=False)
            messagebox.showinfo("Gata", f"Salvat:\n{full_pdf_name}\n{full_xlsx_name}")
